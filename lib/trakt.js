...
function getTitle(episodeObject, callback) {
  ...
  if (show === undefined || season === undefined || episode === undefined) {
    return callback("Not enough episode data to make trakt query!");
  }
  ...
  http.get(options, function (res) {
    ...
    res.on("end", function () {
      ...
      if (index === -1) {
        return callback("Unable to find episode in trakt database");
      }
    ...
    res.on("error", function () {
      return callback("There was a problem connecting to trakt database");
    });
  });
}
