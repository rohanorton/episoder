...
function main(args, callback) {
  // args._ is where yargs puts all arguments not associated with flags
  // apparently yargs also globs filenames.
  ...
  async.each(filenames, function (file, callback) {
    fs.stat(file, function (err, stats) {
      if (err) {
        return callback(new Error("cannot stat file: '" + file + "': No such file or directory"));
      }
      if (stats.isFile()) {
        renameEpisodeFile(file, args, callback);
      } else if (stats.isDirectory()) {
        console.log("Cannot rename " + file + ": Is a directory");
      }
    });
  }, callback);
}
